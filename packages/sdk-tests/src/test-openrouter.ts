...
async function testNonStreaming() {
  log('OpenRouter', 'Testing non-streaming...');
  ...
  try {
    ...
    success('OpenRouter', `Response (${duration}ms): "${result.text.trim()}"`);
    console.log(
      `  Tokens: ${result.usage.inputTokens} prompt, ${result.usage.outputTokens} completion`,
    );
    return true;
  } catch (e: unknown) {
    error('OpenRouter', `Non-streaming failed: ${e instanceof Error ? e.message : String(e)}`);
    return false;
  }
}

async function testStreaming() {
  log('OpenRouter', 'Testing streaming...');
  const start = Date.now();
  ...
  try {
    ...
    success('OpenRouter', `Stream complete (${duration}ms, TTFT: ${firstTokenTime}ms)`);
    console.log(`  Tokens: ${usage.inputTokens} prompt, ${usage.outputTokens} completion`);
    return true;
  } catch (e: unknown) {
    error('OpenRouter', `Streaming failed: ${e instanceof Error ? e.message : String(e)}`);
    return false;
  }
}
...
async function main() {
  ...
  console.log('OpenRouter Proxy Test');
  console.log(`Proxy URL: ${PROXY_URL}/openrouter/v1`);
  console.log('='.repeat(50));
