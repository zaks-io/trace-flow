...
/**
 * One-shot migration for credentials minted before they became valid until revoked.
 * Run in each environment after deploying the optional schema field.
 */
export const removeCollectorCredentialExpiry = internalMutation({
  ...
  handler: async (ctx) => {
    ...
    for (const credential of credentials) {
      if (credential.status !== 'active' || credential.expiresAt === undefined) continue;
