...
describe('collectorCredentials.listActiveForCurrentUser', () => {
  ...
  it('returns the signed-in user active current and legacy unexpired collectors without hashes', async () => {
    ...
    const rows = [
      ...
      {
        _id: 'credential_legacy',
        _creationTime: NOW_MS,
        orgId: 'org_1',
        userId: 'user_1',
        collectorId: 'collector_legacy',
        hashedSecret: 'legacy-hash',
        status: 'active',
        expiresAt: NOW_MS + 1,
      },
      {
    ...
    expect(result).toEqual([
      ...
      expect.objectContaining({ _id: 'credential_legacy', collectorId: 'collector_legacy' }),
    ]);
    for (const credential of result) expect(credential).not.toHaveProperty('hashedSecret');
