...
import { createAccessToken, validateAccessToken, ACCESS_TOKEN_TTL_SECONDS } from './mcp/tokens';
...
app.get('/mcp/authorize', async (c) => {
  ...
  const auth0Url = buildAuth0AuthorizeUrl(state, callbackUrl);
...
app.get('/mcp/callback', async (c) => {
  ...
  try {
    ...
    let auth0Tokens;
    try {
      auth0Tokens = await exchangeAuth0Code(code, callbackUrl);
    } catch (err) {
    ...
    let userInfo;
    try {
      userInfo = await getAuth0UserInfo(auth0Tokens.access_token);
    } catch (err) {
...
app.post('/mcp/token', async (c) => {
  ...
  if (grantType === 'authorization_code') {
    ...
    const accessToken = await createAccessToken(result.userId, result.tokenId);
  ...
  if (grantType === 'refresh_token') {
    ...
    if (refreshToken.auth0RefreshToken) {
      try {
        const newAuth0Tokens = await refreshAuth0Token(refreshToken.auth0RefreshToken);
    ...
    const accessToken = await createAccessToken(refreshToken.userId, refreshTokenId);
