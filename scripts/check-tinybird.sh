...
tb --cloud sql "SELECT TraceId, SpanName, toDateTime(Timestamp / 1000000000) as TimestampUTC FROM otel_trace_spans WHERE SpanName = 'gen_ai.request' ORDER BY Timestamp DESC LIMIT 15"

echo ""
echo "=== Total trace count ==="
tb --cloud sql "SELECT count() as total FROM otel_trace_spans"
