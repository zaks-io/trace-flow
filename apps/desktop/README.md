...
The menu-bar Collector (TRA-115). A Tauri v2 app that runs the same fact-ingest path as the
`trace-flow` CLI: sign in through the browser device flow, store a Collector Credential in the OS
keychain, and sync local Claude Code, Codex CLI, and macOS Cursor activity for `/app/agents`.

macOS arm64 and Windows x64 builds are published, but Agent Conversation Analytics is still
launch-gated by `docs/guides/agent-conversation-analytics/ROADMAP.md`. Distribution does not replace
the remaining normal-user production sync, dashboard, and observability verification.
...
- **Sources** - Claude Code and Codex CLI use their local JSONL stores. Cursor is supported on macOS
  through a read-only snapshot of its global `state.vscdb`; Windows Cursor support is not implemented.
...
Desktop releases run automatically when desktop or shared Collector code lands on `main`. CI signs
and notarizes the macOS arm64 app, builds the Windows x64 installer, signs both platforms' updater
artifacts with Tauri, and publishes them to the public R2 download channel. The manifest is written
last, so a partial upload never becomes the current release.
...
- **Linux desktop target** and **Cursor on Windows/Linux** remain out of scope for the current
  macOS/Windows distribution. Cursor fact ingestion is implemented on macOS.
