...
export function toolEventRow(ctx: BatchContext, fact: AgentToolEventQueueFact) {
  return {
    ...
    status: fact.status,
    error_category: fact.error_category ?? 'unknown',
    error_category_coverage:
      fact.error_category_coverage ?? (fact.status === 'failure' ? 'unknown' : 'not_applicable'),
    exit_code: fact.exit_code ?? 0,
    duration_ms: fact.duration_ms ?? 0,
    is_navigation: fact.is_navigation ? 1 : 0,
    navigation_kind: fact.navigation_kind ?? 'none',
    navigation_hint_coverage: fact.navigation_hint_coverage ?? 'unknown',
    navigation_path_hint: fact.navigation_path_hint ?? '',
    navigation_pattern_hint: fact.navigation_pattern_hint ?? '',
