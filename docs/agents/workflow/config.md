...
- Production deploy path: GitHub Actions on merge to `main` (`deploy-convex` exports `.convex.cloud` and `.convex.site` URLs through `GITHUB_OUTPUT`; Web/Analyst Sandbox consume `.cloud`; Proxy/Agent Ingest/MCP consume `.site`; workers in parallel where dependencies allow; Tinybird schema before the proxy/agent consumers)
