...
Trace Flow Desktop is a menu-bar/tray app with a small settings window. The tray shows compact operational status for detected enabled Sources, last sync, pause/resume, run sync/import controls, recent errors, logs, and quit. Settings owns connect/login, Source enablement, custom Source paths, autostart, diagnostics export, owner-only Archive Export, and the existing Open dashboard link. The website owns Conversation Archive activation and per-Collector enrollment under the existing user session.

For the primary case, a Pro owner opens the dashboard and selects **Enable Conversation Archive** in website settings. The website activates the Organization archive, lists that User's active connected Desktops, and enrolls the selected Collector. Consent requires an explicit **Existing and new conversations** or **New conversations** choice for each selected Source. The first release captures every conversation allowed by the Claude and Codex Source choices without Repo, Project, or conversation filters; Cursor archive capture remains unsupported until its later mutable-store path lands. The enrollment flow discloses that archive content is stored and processed through the Cloudflare R2 `us` jurisdiction. After activation, any current Organization member may enroll their own connected Desktop without another owner-approval step; that flow creates only the User and Collector-specific enrollment and history choice. Activation and enrollment remain distinct server records even when the owner's flow creates both.

Enrollment stores the explicitly authorized archive Source types. A website update that adds Cursor or another archive adapter shows it as available but off for every existing enrollment. The User must explicitly add that Source to the Collector's enrollment and choose **Existing and new conversations** or **New conversations** for it before capture starts. Enabling a new Source never inherits another Source's baseline or history choice.
...
Desktop reports its current archive spool bytes, last archive acknowledgement, and archive error state through the normal authenticated status heartbeat. `/app/agents` displays the latest observation with its timestamp rather than presenting stale local state as live. Website settings is the activation and enrollment surface; Desktop reaches it through the existing Open dashboard link and refreshes read-only policy with its Collector Credential before each serialized cycle.
...
- A Pro owner can complete **Enable Conversation Archive** in website settings; the flow creates Archive Activation, enrolls a caller-owned connected Desktop, requires an explicit history choice for each selected Source, and starts ongoing Claude and Codex archive sync on the Desktop's next normal cycle. New-only completes a durable pre-capture inventory, never archives a baseline session even after later appends, and excludes and reports a later-discovered session with pre-enrollment or ambiguous history.
...
- After Archive Activation, an ordinary member can enroll their own connected Desktop in website settings without separate owner approval or receiving Organization-wide archive authority.
