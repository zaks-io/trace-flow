...
An `archive_integrity_error` response leaves the rejected bytes in the encrypted Archive Spool and pauses archive collection only for the identified Source and Agent Session. Desktop does not advance that session's archive cursor or automatically rewrite, delete, or skip the rejected records. It continues archive collection for other sessions and continues parsed fact sync. The action-required status names the Source and source session identifier without displaying Raw Transcript content and remains until an explicit server-side repair clears it.
