...
4. **`archive.trace-flow.dev` is the sole Conversation Archive data-plane boundary.**
   The planned Archive API accepts enrolled Collector uploads, returns durable archive acknowledgements, streams owner-only lossless exports, executes contribution or whole-archive deletion, and performs non-content integrity verification for an operator-only repair path. It exclusively binds the Agent Archive R2 Standard bucket created with Cloudflare's fixed `us` jurisdiction for the first release, using `jurisdiction: "us"` for the Worker binding and R2 access, and Archive Encryption Key material. The implementation must use a current Cloudflare API or client version that supports this jurisdiction; a non-binding location hint is not equivalent. Uploads require a Collector Credential plus current Collector Enrollment; reads require a short-lived Archive Export Grant minted after interactive owner authentication. Repair authorization cannot return decrypted payloads or act as a reusable archive-read credential. Archive API writes metadata-only Archive Audit Events to Convex through an authenticated internal mutation. It does not bind proxy Body Object secrets, Tinybird credentials, or agent fact queues.
...
- Operator integrity repair can verify and resume only an exact valid chain or pending batch. Its authorization cannot return decrypted payloads, and every attempt writes a metadata-only Archive Audit Event.
