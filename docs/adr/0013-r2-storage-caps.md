...
R2 Infrequent Access could lower stored-byte cost, but retrieval fees make archive export and re-encryption costs usage-dependent. The v1 archive uses Standard storage for a predictable hard ceiling. Packing 100 GB into 1.5 MiB chunks takes about 68,267 Class A writes, roughly $0.31 at the current $4.50 per million Standard Class A operations before shared free-tier allowances. One-object-per-record storage is prohibited because it would make operation count, not stored bytes, the dominant cost.
...
- Archive Chunks contain records from one Archive Contribution and Agent Session, use a 1.5 MiB packing target with an 8 MiB hard limit for a single stored element, and never require cross-contributor reference counting or repacking for deletion.
